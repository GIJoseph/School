...
for item in cart {
    ...
    if item is Ratable {
        let ratableItem = item as! Ratable
        print(ratableItem.averageRating())
    }
    
    if let ratableItem = item as? Ratable {
        print(ratableItem.averageRating())
    }
}
...
for item in cart {
    ...
    case is Ratable:
        let ratableItem = item as! Ratable
        print(item.title, item.priceString, ratableItem.averageRating())
    default:
        break
    }
}
...
cart.forEach {
    if case let item? = $0 as? Purchasable & Ratable {
        print(item.title, item.priceString, item.averageRating())
    }
}

let cart3 = [book, bluRay] as [Any]

let cart4: [AnyObject] = [book, bluRay]

cart4.forEach { print(type(of: $0)) }

let randomSet: Set<AnyHashable> = [1, "Two", 3.0]
...
class Cart {
    ...
    func subtotal() -> Double {
        return delegate.subtotalFor(items: items)
    }
}

protocol CartDelegate {
    
    func subtotalFor(items: [LineItemable]) -> Double
...
shoppingCart.addItem(item: book)
shoppingCart.addItem(item: bluRay)

print(formatter.string(from: NSNumber(value: shoppingCart.subtotal()))!)
