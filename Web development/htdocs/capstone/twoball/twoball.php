<?php
	if(isset($_POST["save"])){
		file_put_contents("twoballprogress.txt", "");
		foreach($_POST["list"] as $num){
			file_put_contents("twoballprogress.txt", $num."\n", FILE_APPEND);
		}
			
	}
	$progress = file("twoballprogress.txt");
	
	
	$file = file("twoball.txt");
	$i = 0;
	$title = array();
	$description = array();
	while($i < count($file)){
	...
	<body>
		...
		<nav>
			<ul>
				...
				<li><a href="../twoball/twoball.php" id="selectedlink">Two Balls</a></li>
		...
		<main>
			<form action="twoball.php" method="post">
				<div id="accordion">
<?php
...
?>			
			...
			<div id="pictureDiv">
				<img src="twoballpictures/twoball.png" alt="stick person" id="picture2">
			</div>	
		</main>
		<footer>
			<hr>
