<?php
	if(isset($_POST["save"])){
		file_put_contents("specialprogress.txt", "");
		foreach($_POST["list"] as $num){
			file_put_contents("specialprogress.txt", $num."\n", FILE_APPEND);
		}
			
	}
	$progress = file("specialprogress.txt");
	
	
	$file = file("special.txt");
	$i = 0;
	$title = array();
	$description = array();
	while($i < count($file)){
	...
	<body>
		...
		<nav>
			<ul>
				...
				<li><a href="../special/special.php" id="selectedlink">Special</a></li>
			</ul>
			<hr>
		</nav>
		<main>
